...
public:
	...
	void show( std::ostream& stream=std::cout, int width=15, int dec=5 ){
		static_cast<const tensor1*>(this)->show(stream,width,dec);
	}
	...
	void show_in_mat( std::ostream& stream=std::cout, int width=15, int dec=5 ){
		static_cast<const tensor1*>(this)->show_in_mat(stream,width,dec);
	}

	//static func
	static tensor1 zero(){
		tensor1 t;
		t.elem[0] = 0.0; t.elem[1] = 0.0; t.elem[2] = 0.0;
		return t;
	}

	static tensor1 constant( const double val ){
		tensor1 t;
		t.elem[0] = val; t.elem[1] = val; t.elem[2] = val;
		return t;
	}
	...
	tensor1 vec_prdct( const tensor1& v );
	double  dot_prdct( const tensor1& v ) const;
	double  dot_prdct( const tensor1& v );
	tensor1 dot_prdct( const sym_tensor2& t2 ) const;
	tensor1 dot_prdct( const sym_tensor2& t2 );
	tensor2 tens_prdct( const tensor1& v ) const;
	tensor2 tens_prdct( const tensor1& v );
